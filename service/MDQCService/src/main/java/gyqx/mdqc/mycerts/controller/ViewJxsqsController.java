...
public class ViewJxsqsController extends BaseController {
    ...
    @GetMapping({"/viewJxsqs/downCerts/{xus}/{goodsId}/{goodsName}", "/mdqcService/viewJxsqs/downCerts/{goodsId}/{goodsName}"})
    public ResponseEntity<Resource> getCertsZip(@PathVariable("goodsId") String goodsId,
                                                @PathVariable("goodsName") String goodsName,
                                                @RequestParam(value = "hosId", required = false) String hosId) throws IOException {
        ByteArrayOutputStream outputStream = service.getCertsZipOutStream(goodsId, hosId);
        ByteArrayResource resource = new ByteArrayResource(outputStream.toByteArray());
        String contentType = "application/zip";
        String filename = goodsName + LocalDate.now().toString() + ".zip";
        return ResponseEntity.ok()
                .contentLength(resource.contentLength())
                .contentType(MediaType.parseMediaType(contentType))
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + URLEncoder.encode(filename, "UTF-8") + "\"")
