...
import gyqx.platform.base.dao.BasCompanyDao;
...
//		XSSFSheet sheet = xwb.getSheet("供应商基本信息");// 供应商信息sheet
        ...
        for (int i = beginLine; counter + beginLine < sheet.getPhysicalNumberOfRows(); i++) {
            ...
            System.out.println(i + "行" + 1 + " 列 is :" + value);
...
//				basCompanyInfo.setIsSupply(row.getCell(21)==null?"":row.getCell(21).toString().trim());
        ...
        for (int i = beginLine; counter + beginLine < sheet.getPhysicalNumberOfRows(); i++) {
            ...
            if (basCompanyCertificates == null || basCompanyCertificates.size() == 0) {
                ...
                System.out.println(basCompanyInfo.getCname() + certKind + "类型长度：" + certKind.length());
...
//					basCompanyCertificateImage.setFileName(row.getCell(7)==null?"":row.getCell(7).toString());
    ...
    @Override
    public BasCompanyInfo addBasCompany(BasCompanyInfo company, boolean addToProv, boolean isProv) throws Exception {
        BasCompanyInfo basCompanyInfo = addBasCompany(company);
        if (addToProv) {
            gyqx.mdqc.custProvManage.vo.BasCompanyInfoVo vo = new gyqx.mdqc.custProvManage.vo.BasCompanyInfoVo();
            BeanUtils.copyProperties(company, vo);
            if(isProv){
                myProvService.addMyProvInfo(Collections.singletonList(vo));
            } else {
                myMfrsService.addMyMfrsInfo(Collections.singletonList(vo));
            }
        }
        return basCompanyInfo;
    }
