<template>
	<el-container style="height:100%">
		<el-header height='40px' class="gy_top-header" v-show="!maxPage"><home></home></el-header>
		<el-container>
				<el-aside :width="getwidth()" v-show="!maxPage">
					<template v-for="(sltNav) in $route.matched" v-if="sltNav.meta.navId">
						<el-menu :default-active="$route.path" class='el-menu-vertical-demo' background-color="#24292e" text-color="#fff" :collapse="collapsed" router unique-opened :key="sltNav.meta.navId">
					...
					<template v-else>
						<el-menu  class='el-menu-vertical-demo' background-color="#24292e" text-color="#fff" :collapse="collapsed" router unique-opened>
...
.el-header{  
  ...
  .breadtoolbar{
		height: 20px;
		background-color:#e5e5e5;//$-bg-color-light;
		.i-btn-collapse{
				float: left;
				font-size: 16px;
				cursor:pointer;
