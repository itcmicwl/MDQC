<template>
  <div id="myCompanyInfo" style="height: 100%;overflow:auto;">
          ...
          <fieldset style="border: 1px solid rgb(97, 192, 254);margin-bottom: 5px;background-color: #e0ebef38;">
            ...
            <el-form :model="company" label-width="100px" :rules="editFormRules" ref="editForm"
              ...
              <el-row>
                <!--<el-col :span="8">-->
                  <!--<el-form-item label="我是厂家" :error="errors.codeErr">-->
                    <!--<el-checkbox v-model="ifMfrs" @change="changeIfMfrs"></el-checkbox>-->
                  <!--</el-form-item>-->
                <!--</el-col>-->
                <!--<el-col :span="8" >-->
                  <!--<el-form-item label="厂家类型" label-width="99px" prop="mfrsKind" :error="errors.codeErr">-->
                    <!--<template>-->
                      <!--<el-radio-group :disabled="!ifMfrs" v-model="company.mfrsKind" @change="changeMfrsKind" size="mini">-->
              ...
              <el-col :span="5" style="margin-left: -16px;">
                <el-button :loading="editLoading" @click="handleBtnEditClick" plain :icon="buttonIcon" :type="buttonType">
                  {{editDisable ? '编辑' : '保存'}}
                </el-button>
                <el-button v-if="editDisable === false" @click="handleBtnCancleClick" plain icon="el-icon-arrow-left" type="warning">取消
                </el-button>
                <!--<el-button type="success" size="mini" @click="saveCompanyInfo" plain icon="el-icon-check">保存</el-button>-->
              </el-col>
            </el-form>
    ...
    <el-row style="max-width: 960px;margin:0 auto">
      <el-col :span="24">
        <el-button type="text" plain @click="addProvCerts">新增证照</el-button>
        <el-col>
          <el-table :data="company.companyCertificateVos" highlight-current-row  style="width: 100%;" border fit><!--:row-class-name="tableRowClassName"-->
                  ...
                  <el-col :span="12">
                    <el-row>
                      <a :href='imgBasePath+scope.row.basCompanyCertificateImages[i-1].filePath'
                        ...
                        <el-tag type="danger" color="#e96146" size="small" v-if="scope.row.certKind=='2'"
                                style="color: white;font-weight: bold" class="box_shadow_class">执业许可
                        </el-tag>
                        ...
                        <el-tag type="success" color="#e63533" size="small" v-if="scope.row.certKind=='7'"
    ...
    methods: {
      ...
      handleBtnEditClick () {
        if (!this.editDisable) {
          ...
          this.buttonIcon = 'el-icon-check';
          this.buttonType = 'success';
          this.editDisable = !this.editDisable;
        }
      },
      changeMfrsKind:function (d1,d2,d3) {
      },
      changeIfMfrs:function (d1,d2,d3) {
        if(d1){// 勾选我是厂商，则机构类型为'4-厂供一体'
          this.company.kind = '4';
        }else{// 取消勾选我是厂商，则机构类型为'2-供应商'，厂商类型置空
          this.company.mfrsKind = '';
          this.company.kind = '2';
        }
      },
      cancel:function () {
      ...
      deleteProvCerts:function (row) {
        this.$confirm('确认删除吗?', '提示', {}).then(() => {
          this.axios.post('/mdqcService/myInfo/companyInfo/deleteProvCerts', row).then(res => {
            if (res.data.code == "0") {
              this.getCompanyInfo(this.corpId);
      ...
      deleteCert:function (file, fileList) {
        ...
        this.axios.post('/mdqcService/myInfo/companyInfo/deleteImage', {id:file.id}).then(res => {
          if (res.data.code == "0") {
            this.getCompanyInfo(this.corpId);
            this.$message({message: '操作成功！', type: 'success'});
          }else{
      ...
      getCompanyInfo: function (corpId) {
        this.loading = true;
        var _this = this;
        this.axios.post('/mdqcService/myInfo/companyInfo/getCompanyInfo', corpId).then(res => {
          this.loading = false;
          if (res.data.code == "0") {
      ...
      saveCompanyInfo:function () {
        ...
        let serviceUrl = "/mdqcService/myInfo/companyInfo/updateCompanyCertificate";
      ...
      getZZLX: function () {   //证照类型 字典
        ...
        this.axios.post('/platformService/sys/dict/getDictValueByDictEname', { dictName: 'ZZLX' }).then(res => {
          ...
          if (res.data.code == "0") {
            ...
            let tempArray = [];
            if(this.company.kind=='3'){// 如果机构类型是'3-厂商'，则取厂商证照类型字典
...
<style>
  ...
  #myCompanyInfo .expiring {
    /*color: #ff47e8;*/
    color: #E6A23C;
  }
  #myCompanyInfo .expired {
    color: #ff311f;
  }
  #myCompanyInfo .el-table .warning-red {
    color: #F56C6C;
  }
  #myCompanyInfo .el-table .warning-row {
    color: #E6A23C;
  }
  #myCompanyInfo .el-table .warning-success {
    color: #57b547;
  }
</style>
