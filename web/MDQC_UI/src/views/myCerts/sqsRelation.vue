     ...
     contextMenuClick(cmdKind){
      ...
      removeRelation(){
        ...
        if(this.currentSqs.id == this.jxsqsRelation.id){
          ...
          msg += '</table>';
          cb=function(){
            _this.axios.post(`/mdqcService/provJxsqs/delRelation/${_this.currentSqs.id}`,param).then(res=>{
                if(res.data.code==0){
      ...
      getSqsRelation(sqsId){
        var _this = this;
        myChart.showLoading();
        this.axios.get(`/mdqcService/provJxsqs/getSqsRelationById/${sqsId}`).then(res=>{
