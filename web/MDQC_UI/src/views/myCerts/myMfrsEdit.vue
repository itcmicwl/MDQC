<template>
  <div id="mfrsEdit" style="height: 100%;overflow:auto;">
        ...
        <div>
          <fieldset style="border: 1px solid rgb(97, 192, 254);;margin-bottom: 5px;background-color: #e0ebef38;">
            ...
            <el-form :model="company" label-width="100px" :rules="editFormRules" ref="editForm"
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item label="我是供应商" :error="errors.codeErr">
                    <el-checkbox v-model="ifProv" disabled></el-checkbox>
                  </el-form-item>
                </el-col>
                <el-col :span="8" >
                  <el-form-item label="厂家类型" label-width="99px" prop="mfrsKind" :error="errors.codeErr">
                    <template>
                      <el-radio-group  v-model="company.mfrsKind" disabled size="mini">
    ...
    <el-row style="max-width: 960px;margin:0 auto">
      <el-col :span="24">
        ...
        <el-col>
          <el-table :data="company.provMfrsCertificateVo" highlight-current-row  style="width: 100%;" border fit>
            ...
            <el-table-column label="操作" width="100" align="center">
              <template slot-scope="scope">
                <el-button  @click="updateMfrsCerts(scope.row)" type="text">编辑</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      cancel:function () {
        this.$router.push({ path: 'myMfrs',query:{cname:this.cname}});
      },
      ...
      updateCerts () {
        ...
        let serverUrl = "/mdqcService/myCerts/myMfrs/updateMfrsCerts";
      ...
      handleProvCertsSuccess(response, file, fileList) {
        ...
        this.companyCerts.imageList.push({filePath:certPath});
      ...
      addImage (imageObj) {
        let serverUrl = "/mdqcService/myCerts/myMfrs/addImage";
      ...
      updateMfrsCerts:function (d1) {
        this.fileList = [];
        this.companyCerts = d1;
        d1.imageList.forEach(item=>{
      ...
      deleteProvCerts:function (row) {
        this.$confirm('确认删除吗?', '提示', {}).then(() => {
          this.axios.post('/mdqcService/myCerts/myMfrs/deleteMfrsCerts', row).then(res => {
            if (res.data.code == "0") {
              this.$message({message: '操作成功！', type: 'success'});
              let tempArray = this.company.provMfrsCertificateVo;
      ...
      deleteCert:function (file, fileList) {
        ...
        this.axios.post('/mdqcService/myCerts/myMfrs/deleteImage', {id:file.id}).then(res => {
          if (res.data.code == "0") {
            this.$message({message: '操作成功！', type: 'success'});
            this.fileList = fileList;
            let images = this.companyCerts.imageList;
      ...
      dlgConfirm () {
        this.$refs.cnsForm.validate((valid) => {
          if (valid) {
            this.dlgConfirmBtnLoading = true;
            let serverUrl = "/mdqcService/myCerts/myMfrs/addMyMfrsCerts";
            this.companyCerts.pId = this.company.id;
            this.companyCerts.mfrsId = this.company.mfrsId;
            this.axios.post(serverUrl, this.companyCerts).then(res => {
              if (res.data.code >= 0) {
                this.$message.success('操作成功。');
                this.company.provMfrsCertificateVo.push(res.data.data);
                this.dlgConfirmBtnLoading = false;
      ...
      formateCertType (d1,d2,d3){
        let that = this;
        // let obj = that.certKinds.find(item => item.val == d3);
        let obj = that.allCertKinds.find(item => item.val == d3);
    ...
    mounted() {
      ...
      if(tempObj.company){
        ...
        // this.certKinds = tempObj.certKinds;
        this.ifProv = (tempObj.company.kind == '2'||tempObj.company.kind == '4')?true:false;
      }
