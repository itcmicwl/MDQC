  ...
  data() {
    let validateBeginDate = (rule, value, callback) => {
      if (!value) {
        ...
        if (
          this.currentJxsqs.expdtEndDate &&
          !(this.currentJxsqs.isValiding === "1")
        ) {
    ...
    let validateEndDate = (rule, value, callback) => {
      if (!value && !(this.currentJxsqs.isValiding === "1")) {
        ...
        if (
          Date.parse(this.currentJxsqs.expdtBeginDate) >
          Date.parse(this.currentJxsqs.expdtEndDate)
        ) {
    ...
    return {
      validateRules: {
        ...
        chiefId: [{ validator: validateChief, trigger: "change" }],
  ...
  methods: {
    ...
    handleEdit(item) {
      ...
      if (item.filePath) {
        ...
        if (slashIndex !== -1) {
          this.imageUrl =
            this.picURLPrefix +
            item.filePath.substring(0, slashIndex) +
            "/thumb-" +
            item.filePath.substring(slashIndex + 1);
        } else {
          this.imageUrl = "";
        }
      } else {
      ...
      this.mfrsSearchData = [
        { id: this.currentJxsqs.mfrsId, mfrsName: this.currentJxsqs.mfrsName }
      ];
      this.superiorCompanySearchData = [
        {
      ...
      this.$nextTick(() => {
        this.$refs["hosTable"].clearSelection();
        this.$refs["certsTable"].clearSelection();
        this.$refs["hosTable"].store.states.selection.push(
          ...this.currentJxsqs.forHospitals
        );
        this.$refs["certsTable"].store.states.selection.push(
          ...this.currentJxsqs.relatedCerts
        );
      });
    },
    handleDelete(item) {
          ...
          this.axios
            ...
            .then(res => {
              if (res.data.code >= 0) {
                ...
                const removeIndex = this.jxsqsList.findIndex(
                  o => o.id === item.id
                );
    ...
    handleZjzjDelete(item) {
      ...
      this.$confirm("此操作将删除该记录, 是否继续?", "提示", {
        ...
        .then(_ => {
          if (!this.dlgAddMode) {
            this.axios
              .delete(this.baseURL + "/delProvJxsqsZjzj", {
              ...
              .then(res => {
                if (res.data.code >= 0) {
                  ...
                  const currentRow = this.jxsqsList.find(
                    o => o.id === that.currentJxsqs.id
                  );
                  const removeIndex1 = currentRow.zjzjList.findIndex(
                    o => o.id === item.id
                  );
                  if (removeIndex1 !== -1) {
                    currentRow.zjzjList.splice(removeIndex1, 1);
                  }
                  const removeIndex2 = that.currentJxsqs.zjzjList.findIndex(
                    o => o.id === item.id
                  );
                  if (removeIndex2 !== -1) {
            ...
            const removeIndex = this.currentJxsqs.zjzjList.findIndex(
              o => o.pId === item.pId
            );
            if (removeIndex !== -1) {
      ...
      this.$refs["jxsqsForm"].validate(valid => {
        if (valid) {
          ...
          if (this.dlgAddMode) {
            // new
            this.axios
              .post(this.baseURL + "/addJxsqs", this.handleData)
              .then(res => {
                if (res.data.code >= 0) {
            ...
            // modify
            this.axios
              .put(this.baseURL + "/modJxsqs", this.handleData)
              .then(res => {
                if (res.data.code >= 0) {
    ...
    zjzjDlgConfirm() {
      ...
      this.$refs["zjzjForm"].validate(valid => {
        ...
        if (valid) {
          if (
            that.currentZjzj.pname === that.currentJxsqs.superiorMfrsName ||
            that.currentJxsqs.zjzjList.findIndex(
              o => o.pname === that.currentZjzj.pname
            ) !== -1
          ) {
          ...
          if (!that.dlgAddMode) {
            ...
            that.axios
              .post(that.baseURL + "/addProvJxsqsZjzj", that.currentZjzj)
              .then(res => {
                if (res.data.code >= 0) {
                  that.currentZjzj.id = res.data.data;
                  const currentRow = that.jxsqsList.find(
                    o => o.id === that.currentJxsqs.id
                  );
                  currentRow.zjzjList.push(
                    JSON.parse(JSON.stringify(that.currentZjzj))
                  );
                  that.currentJxsqs.zjzjList.push(
                    JSON.parse(JSON.stringify(that.currentZjzj))
                  );
                  ...
                  that.$message.error(
                    "新增信息失败，" + res.data.msg.substring(0, 50) + "..."
                  );
                }
              })
              .catch(err => {
    ...
    handlePicSuccess(response, file, fileList) {
      if (response.code < 0) {
        this.$message({
          ...
          duration: 0,
          message:
            file.name + "上传失败，" + response.msg.substring(0, 50) + "..."
        });
      } else {
    ...
    addItem(item, type) {
      if (type === "cert") {
        if (
          this.currentJxsqs.relatedCerts.findIndex(o => o.id === item.id) !== -1
        )
          return;
        this.currentJxsqs.relatedCerts.push(item);
        const removeIndex = this.currentJxsqs.delCerts.findIndex(
          o => o.id === item.id
        );
        if (removeIndex !== -1)
          this.currentJxsqs.delCerts.splice(removeIndex, 1);
        else this.currentJxsqs.addCerts.push(item);
      } else if (type === "hos") {
        if (
          this.currentJxsqs.forHospitals.findIndex(o => o.id === item.id) !== -1
        )
          return;
        this.currentJxsqs.forHospitals.push(item);
        const removeIndex = this.currentJxsqs.delHos.findIndex(
          o => o.id === item.id
        );
        if (removeIndex !== -1) this.currentJxsqs.delHos.splice(removeIndex, 1);
        else this.currentJxsqs.addHos.push(item);
      }
    },
    removeItem(item, type) {
      if (type === "cert") {
        const removeIndex1 = this.currentJxsqs.relatedCerts.findIndex(
          o => o.id === item.id
        );
        if (removeIndex1 !== -1)
          this.currentJxsqs.relatedCerts.splice(removeIndex1, 1);
        const removeIndex2 = this.currentJxsqs.addCerts.findIndex(
          o => o.id === item.id
        );
        if (removeIndex2 !== -1)
          this.currentJxsqs.addCerts.splice(removeIndex2, 1);
        else this.currentJxsqs.delCerts.push(item);
      } else if (type === "hos") {
        const removeIndex1 = this.currentJxsqs.forHospitals.findIndex(
          o => o.id === item.id
        );
        if (removeIndex1 !== -1)
          this.currentJxsqs.forHospitals.splice(removeIndex1, 1);
        const removeIndex2 = this.currentJxsqs.addHos.findIndex(
          o => o.id === item.id
        );
        if (removeIndex2 !== -1)
          this.currentJxsqs.addHos.splice(removeIndex2, 1);
        else this.currentJxsqs.delHos.push(item);
      }
    },
    ...
    debounceQuerySearchCompanyAsync: debounce(500, function(queryString) {
      this.axios
        .get(this.baseURL + "/getBasCompany", {
          params: {
            name: queryString
          }
        })
    ...
    debounceQuerySearchZjzjCompanyAsync: debounce(500, function(queryString) {
      this.axios
        .get(this.baseURL + "/getZjzjCompany", {
          params: {
            name: queryString
          }
        })
    ...
    getProvRegInfo(sqsId) {
      ...
      this.axios
        .get(this.baseURL + "/getProvRegInfo", {
          params
        })
        .then(res => {
          if (res.data.code >= 0) {
            this.provMfrsCertsList = res.data.data.data;
            this.provMfrsCertsList.forEach(
              item => (item.mfrsName = this.currentJxsqs.mfrsName)
            );
...
#provJxsqs .el-upload-list__item,
