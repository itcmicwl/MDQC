<template>
    <div id="provLZXYS" style="height:100%">
        <el-row  class="queryFormClass">
            ...
            <div class="inline" style="margin-left: 5px;">
              ...
              <el-button type="success" icon="el-icon-plus" plain style="height: 26px;margin-left: 0px;" @click="addPrepare" >新增协议书</el-button>
            </div>
        </el-row>
        <el-table highlight-current-row ref="cnsTable" class="provCns-tableheight" :row-class-name="tableRowClassName"
            ...
            <el-table-column label="协议书编号" prop="id" width="120">
            </el-table-column>
        ...
        <el-dialog :title="dlgAddMode?'添加廉政协议书':'廉政协议书信息'" :visible.sync="dlgVisible" >
            <el-form :model="currentCns" ref="cnsForm" label-width="100px" :rules="validateRules"
              ...
              <el-row>
                ...
                <el-form-item label="廉政协议书" prop="filePath" required>
                    <el-input v-model="currentCns.filePath" style="display:none" readonly
                              disabled></el-input>
...
export default {
  data () {
    ...
    return {
      validateRules: {
        ...
        filePath: [
          {required: true, message: '请上传谦政协议书', trigger: 'change'}
        ]
      },
      baseURL: '/mdqcService/provIncorruptAgrmt',
  ...
  computed: {
    largeCnsImageUrl () {
      if (this.currentCns.filePath && this.showBigPic && this.uploadSuccess) {
        return this.picURLPrefix + this.currentCns.filePath
      } else {
  ...
  methods: {
    ...
    handleHosSelect (item) {
      if (item) {
        ...
        if (hos) { this.currentCns.hosName = hos.hosName }
      }
    },
    handleFreezeRemote () {
    ...
    handleEdit (item) {
      ...
      if (item.filePath) {
        ...
        if (slashIndex !== -1) {
          this.cnsImageUrl = this.picURLPrefix + item.filePath.substring(0, slashIndex) + '/thumb-' + item.filePath.substring(slashIndex + 1)
    ...
    handleDelete (item) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        this.axios.delete(this.baseURL + '/delIncorruptAgrmt', {
          params: {
            'id': item.id,
    ...
    handleCnsSuccess (response, file, fileList) {
      if (response.code < 0) {
        ...
        this.currentCns.filePath = path + '/' + id + '.' + ext
        this.$refs.cnsForm.validateField('filePath')
        this.$nextTick(() => {
          this.uploadSuccess = true
        })
      }
      ...
      this.$refs.cnsForm.validate((valid) => {
        if (valid) {
          ...
          if (this.dlgAddMode) { // new
            this.axios.post(this.baseURL + '/addIncorruptAgrmt', this.handleData).then(res => {
              if (res.data.code >= 0) {
                this.$message.success('新增廉政协议书成功。')
                ...
                this.$message.error('新增廉政协议书失败，' + res.data.msg.substring(0, 50) + '...')
              }
            }).catch(_ => {
              this.$message.error('数据异常，请联系管理员！')
            })
          } else { // modify
            this.axios.put(this.baseURL + '/modIncorruptAgrmt', this.handleData).then(res => {
              if (res.data.code >= 0) {
    ...
    getCns () {
      ...
      if (this.searchExpdtBeginDate !== '') { params.start = Date.parse(this.searchExpdtBeginDate) }
      if (this.searchExpdtEndDate !== '') { params.end = Date.parse(this.searchExpdtEndDate) }
      this.axios.get(this.baseURL + '/getIncorruptAgrmt', {
        params
      }).then(res => {
        if (res.data.code >= 0) {
    ...
    changeTableData (item) {
      ...
      let {hosId, hosName, filePath, expdtBeginDate, expdtEndDate, version} = item
      cns.hosId = hosId
      cns.hosName = hosName
      cns.filePath = filePath
...
<style>
    ...
    #provLZXYS .el-date-editor.el-input {
        width: 120px;
    }
    ...
    #provLZXYS .pic-uploader .el-upload {
    ...
    #provLZXYS .upload-background .el-upload::after {
        content: '';
        display: block;
        width: 100%;
        height: 100%;
    ...
    #provLZXYS .pic-uploader .el-upload:hover {
        border-color: #20a0ff;
    }

    #provLZXYS .pic-uploader .item-status-label {
        position: absolute;
        right: -15px;
        top: -6px;
        width: 40px;
    ...
    #provLZXYS .pic-uploader .item-status-label .upload-success {
        font-size: 12px;
        margin-top: 11px;
    ...
    #provLZXYS .el-upload-list__item, .el-upload-list__item:first-child {
        margin-top: 0;
        margin-bottom: 5px;
    }
    ...
    #provLZXYS .el-dialog__footer {
      padding: 10px 20px 20px;
      text-align: center;
      -webkit-box-sizing: border-box;
      box-sizing: border-box;
    }
    #provLZXYS .el-table .warning-red {
      color: #F56C6C;
    }
    #provLZXYS .el-table .warning-row {
      color: #E6A23C;
    }
</style>
