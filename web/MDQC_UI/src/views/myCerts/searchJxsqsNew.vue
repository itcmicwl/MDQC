  ...
  export default {
    ...
    methods: {
        ...
        getSqsRelation(sqsId){
            return this.axios.get(`/mdqcService/provJxsqs/getSqsRelationById/${sqsId}`);
        },
        afterSubmit(){
        ...
        submitRelation(param){
            var _this = this;
            _this.axios.post(`/mdqcService/provJxsqs/saveRelation/${_this.sqsBill.id}`,param).then(res=>{
                if(res.data.code == 0){
