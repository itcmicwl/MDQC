  ...
  <div id="view-jxsqs" class="full-height overflow">
    ...
    <transition name="el-fade-in-linear">
      <div v-show="bShowTrackingChain">
        ...
        <el-button style="margin:0 10px 0 0;" type="primary" icon="el-icon-arrow-left" @click="bShowTrackingChain = false">返回</el-button>
        <el-button v-if="!bIsHos" style="margin:10px 10px 0 0;" type="success" icon="el-icon-download
" @click="downloadAttachments(curGoods)">下载附件</el-button>
      </div>
    </transition>
  </div>
</template>
...
export default {
  data() {
    return {
      BASE_URL: "/mdqcService/viewJxsqs",
  ...
  methods: {
    ...
    downloadAttachments(curGoods) {
      let url = `${this.BASE_URL}/downCerts/X-US-${this.user.xus}/${
        curGoods.spdGoodsCode
      }/${curGoods.goodsName}`;

      if (this.filter.hosId) {
        url = url + "?hosId=" + this.filter.hosId;
      }
