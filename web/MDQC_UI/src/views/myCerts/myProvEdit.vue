  ...
  export default {
    ...
    methods: {
      ...
      updateCerts () {
        ...
        let serverUrl = "/mdqcService/myCerts/myProv/updateProvCerts";
      ...
      addImage (imageObj) {
        let serverUrl = "/mdqcService/myCerts/myProv/addImage";
      ...
      deleteProvCerts:function (row) {
        this.$confirm('确认删除吗?', '提示', {}).then(() => {
          this.axios.post('/mdqcService/myCerts/myProv/deleteProvCerts', row).then(res => {
            if (res.data.code == "0") {
      ...
      deleteCert:function (file, fileList) {
        ...
        this.axios.post('/mdqcService/myCerts/myProv/deleteImage', {id:file.id}).then(res => {
          if (res.data.code == "0") {
            this.$message({message: '操作成功！', type: 'success'});
      ...
      dlgConfirm () {
        this.$refs.cnsForm.validate((valid) => {
          if (valid) {
            this.dlgConfirmBtnLoading = true;
            let serverUrl = "/mdqcService/myCerts/myProv/addMyProvCerts";
