  ...
  <div id="myProv" style="height: 100%;">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" label-width="88px" class="queryFormClass">
        ...
        <el-form-item>
          <el-button type="primary" @click="getMyProvInfoList(1)" plain style="width: 80px;height:28px;" icon="el-icon-search">搜索</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="success" icon="el-icon-plus" plain style="height: 28px" @click="provAdd()" >导入基础供应商</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <!--列表-->
    <template>
      <el-table :data="myProvs" highlight-current-row v-loading="loading" border style="width: 100%;" fit class="myProv-tableheight" height="outer">
      ...
      <el-col :span="24" class="toolbar">
        <el-pagination @size-change="mhandleSizeChange" @current-change="mhandleCurrentChange" :current-page="mpage" :page-sizes="[30, 100, 200, 400]" :page-size="mpageSize" layout="total, sizes, prev, pager, next" :total="mtotal" style="float:right;" small>
        </el-pagination>
      </el-col>
    </template>
    <el-dialog title="选择供应商" :visible.sync="provInfoVisible" top="6vh" width="75%">
      <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        <el-form :inline="true" label-width="88px" class="queryFormClass">
          ...
          <el-form-item>
            <el-button type="primary" @click="getProvInfoList(1)" plain style="width: 80px;height:28px;" icon="el-icon-search">搜索</el-button>
            <el-button type="success" icon="el-icon-plus" plain style="height: 28px" @click="provAddBase(null)" >新增供应商</el-button>
          </el-form-item>
        </el-form>
      </el-col>
      <template>
        <el-table :data="provs" ref="provsTable" highlight-current-row v-loading="loading" border style="width: 100%;" fit max-height="400" @select="handleProvsSelectionChange">
    ...
    <!--Excel导入界面-->
    <el-dialog title="从Excel中导入供应商模板" close="onClose" :visible.sync="importExcelVisible" :show-close="true" :close-on-press-escape="true" :close-on-click-modal="true">
      <template>
        <div>
          <el-upload
            class="upload-demo"
...
export default {
  ...
  methods: {
    ...
    importExcelSubmit: function() {
      ...
      this.axios
        .post("/mdqcService/bas/company/importCompanyCertInfos", this.fileMap)
        .then(
          res => {
    ...
    provEdit: function(row) {
      this.$router.push({
        path: "myProvEdit",
        query: { company: row, cname: this.myFilters.cname }
      });
    },
    provAdd: function() {
    ...
    confirmSelectProvs: function() {
      ...
      this.axios.post("/mdqcService/myCerts/myProv/addMyProvInfo", param).then(
        res => {
          if (res.data.code == "0") {
    ...
    getProvInfoList: function(pIndex) {
      ...
      this.loading = true;
      this.axios
        .post("/mdqcService/custProvManage/provInfo/getProvInfoList", param)
        .then(
          res => {
            this.loading = false;
            if (res.data.code == "0") {
    ...
    getMyProvInfoList: function(pIndex) {
      ...
      var param = {
        orderBy: "",
        queryObject: {
          cname: this.myFilters.cname,
          flag: this.myFilters.flag,
          collectorId: this.user.corpId
        },
        page: this.mpage,
        rows: this.mpageSize
      };
      this.myLoading = true;
      this.axios.post("/mdqcService/myCerts/myProv/getMyProvInfo", param).then(
        res => {
    ...
    updateMyProvFlag(id, flag) {
      let message =
        flag === "0" ? "确认停用这条记录吗?" : "确认启用这条记录吗?";
      this.$confirm(message, "提示", {})
        .then(() => {
          this.updateFlagLoading = true;
          this.axios
            .post("/mdqcService/myCerts/myProv/updateMyProvFlag", {
              id: id,
              flag: flag
            })
            .then(
    ...
    provAddBase() {
      this.$router.push({
        path: "provEdit",
        query: { editDisable: "1", addMyProv: true }
      });
    }
  }
};
