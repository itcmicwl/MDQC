<template>
  <div id="provEdit" style="height: 100%;overflow:auto;">
          ...
          <fieldset style="border: 1px solid rgb(97, 192, 254);;margin-bottom: 5px;background-color: #e0ebef38;">
            ...
            <el-col :span="24">
              ...
              <el-col :span="4" style="margin-left: -13px;">
                <el-button type="primary" size="mini" @click="$router.go(-1)" plain icon="el-icon-back">返回</el-button>
                <!-- cancel -->
...
export default {
  ...
  mounted() {
    ...
    if (tempObj.addMyProv) {
      this.addMyProv = tempObj.addMyProv;
    }
  ...
  methods: {
    ...
    changeIfMfrs: function(d1, d2, d3) {
      if (d1) {
        // 勾选我是厂商，则机构类型为'4-厂供一体'
        this.company.mfrsKind = "1"; // 厂商默认类型为境内
        this.company.kind = "4";
      } else {
        // 取消勾选我是厂商，则机构类型为'2-供应商'，厂商类型置空
    ...
    saveCompanyInfo: function() {
      ...
      if (this.addMyProv) {
        serviceUrl = serviceUrl + "?isProv=true&addToProv=" + this.addMyProv;
      }
      if (this.company.id) {
